...
if ($data !== null) {
    ...
    $dropped_items = $data['droppedItems'];
    $updateFields = [];
    $insertUserTanks = [];
    $existingTanks = [];
    $goldToAdd = 0;
    ...
    try {
        ...
        foreach ($data['droppedItems'] as $item) {
            if ($item['type'] !== 'tank') {
                ...
                if (!$stmtCheck) {
                    throw new Exception('Failed to prepare check tank query: ' . $conn->error);
                }
                ...
                if ($resultCheck->num_rows > 0) {
                    ...
                    if (!$stmtConversion) {
                        throw new Exception('Failed to prepare conversion query: ' . $conn->error);
                    }
        ...
        if (!empty($updateFields) || $goldToAdd > 0) {
            ...
            if (!empty($updateFields)) {
                ...
                if ($stmt->execute()) {
                    ...
                    throw new Exception('Failed to execute user update query: ' . $stmt->error);
                }
                $stmt->close();
            } else {
                $response = [
                    'status' => 'error',
                    'message' => 'No update fields specified',
                ];
            }
        }

        // Вставка нових танків
        if (!empty($insertUserTanks)) {
            ...
            if ($conn->query($insertQuery) === TRUE) {
                ...
                $response['updated_dropped_items'] = $data['droppedItems'];
                if (!empty($convertedItems)) {
                    $response['converted_items'] = $convertedItems;
                }
                if (!empty($newDroppedTanks)) {
                ...
                throw new Exception('Failed to insert user tanks: ' . $conn->error);
            }
        }
