...
export function ModalStorage({ setIsUpdated, playerData, addMessage }) {
   ...
   const [tooltip, setTooltip] = useState({ visible: false, text: "", x: 0, y: 0, id: null });
   const [currentTooltipId, setCurrentTooltipId] = useState(null);

   const handleSetTooltip = (tooltipData) => {
      setTooltip(tooltipData);
      setCurrentTooltipId(tooltipData.id);
   };
         ...
         {playerData && (
            <ModalStorageLayout
               ...
               goldItem={
                  <Tooltip
                     id="gold"
                     text='Золото <img src="img/resources/gold.png" alt="RES" /> - один з найголовніших ресурсів в грі. За допомогою золота можна покупати танки, скіни і кейси. Також в розділі "Обмін" <img src="img/servise/exchange.png" alt="EXCHANGE" /> ви можете отримати більше Золота, обмінявши інші ресурси.'
                     tooltipId={currentTooltipId}
                     setTooltipId={setCurrentTooltipId}
...
function Tooltip({ id, tooltip, tooltipId, setTooltipId, setTooltip, text, children }) {
   const handleMouseEnter = (event) => {
      const { clientX: x, clientY: y } = event;
      setTooltipId(id);
      setTooltip({ visible: true, text, x, y, id });
   };
   ...
   const handleMouseLeave = () => {
      if (tooltipId === id) {
         setTooltip({ visible: false, text: "", x: 0, y: 0, id: null });
         setTooltipId(null); // Clear the tooltipId when hiding
      }
   };

   // Ensure tooltip is defined and has the correct properties
   const tooltipVisible = tooltip && tooltip.visible;
   ...
   return (
      <>
         ...
         {tooltipVisible && isCurrentTooltip && (
            <span
